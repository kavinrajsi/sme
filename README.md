...
Marketing site for **SearchMadarth**, a digital marketing partner for Indian SMEs. Features a public marketing site with quiz-driven lead capture and demo request modal, a Payload CMS admin panel for case studies, a Neon Postgres-backed submissions viewer, and machine-readable surfaces (sitemap, robots, llms.txt) for SEO/AEO/AIO.
...
| Database | [Neon](https://neon.tech/) (Postgres) | One project hosts both Payload's tables and the form-submission / OTP tables, all accessed over one `DATABASE_URI` connection string via `pg` |
...
All forms post to **server actions** (no API routes). Each one verifies the user's email via the OTP flow before sending the notification email and writing to Postgres.
...
| `DATABASE_URI` | yes | Postgres connection string, used by both Payload's adapter and `src/lib/db.js` (form/OTP tables + the `/api/health/db` probe). Use Neon's **pooled** connection string (hostname contains `-pooler`). |
...
| `DATABASE_URI` | yes (for CI) | Neon connection string the smoke test uses to inject OTP rows and poll form tables directly |
| `OTP_PEPPER` | yes (for CI) | Must match the deployed target's `OTP_PEPPER` so the smoke test's injected OTP hash verifies |
| `SMOKE_TARGET_URL` | no | Override the URL the smoke test hits. Defaults to `https://sme.searchmadarth.com` |

Set these in **Settings → Secrets and variables → Actions** on the GitHub repo. Adding them to `.env.local` also lets you run `npm run smoke:forms` locally — see [Smoke tests](#smoke-tests).
...
- OTP: `OTP_PEPPER`
- Payload / Database: `PAYLOAD_SECRET`, `DATABASE_URI`, `BLOB_READ_WRITE_TOKEN`
...
| Required secrets | `BROWSERBASE_API_KEY`, `BROWSERBASE_PROJECT_ID`, `DATABASE_URI`, `OTP_PEPPER` in repo Actions secrets |
...
To run locally: ensure `BROWSERBASE_API_KEY`, `BROWSERBASE_PROJECT_ID`, `DATABASE_URI`, and `OTP_PEPPER` are in `.env.local`, then `npm run smoke:forms`. Use `SMOKE_TARGET_URL=http://localhost:3000 npm run smoke:forms` to point at a local dev server (set `EMAIL_DISABLED=true` to avoid sending real mail).
